...
(function() {
  ...
  const fruitname = options => {
    const left = random(name);
    const right = random(fruit);
    const possessive = left.charAt(left.length - 1) === "s" ? "' " : "'s ";
    if (options && options.safe === true) {
      return (
        left.toLowerCase() +
        possessive.replace("'", "").replace(" ", "_") +
        right.toLowerCase()
      );
    } else return left + possessive + right;
  };
